...
1. Download and install (`nvs add`) the newer version, if not already added.
1. Migrate global packages (`nvs migrate`) from the old to the new version.
1. If the old version was linked, link (`nvs link`) the new version instead.
1. Use (`nvs use`) the new version if the old version was previously in use.
