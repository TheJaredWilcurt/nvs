...
function findAutoVersionAsync(cwd) {
	...
	function findDevEngines(directory, versionString) {
	...
	function findVoltaObject(directory, versionString) {
		if (directory && !versionString) {
			const manifest = getManifest(directory);
			if (
				manifest &&
	...
	while (dir) {
		...
		let versionString;
		versionString = findDevEngines(dir, versionString);
		versionString = findVoltaObject(dir, versionString);
